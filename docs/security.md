...
- Before the implementation agent runs, persisted `actions/checkout` headers, helpers, and authenticated origin URLs are removed from local Git configuration. Control-plane fetch/push uses an explicit short-lived token path after the agent exits.
...
- Commands are screened across operator-separated segments (so `make && curl …` is caught). The workflow owner alone configures the lists.
...
Changes are approval-gated by default. — [`src/github/ops.ts`](../src/github/ops.ts)

Keep `require_push_approval: true` (the default):
...
- Deepagents filesystem writes under `/.deepagents/**` are denied before custom permission rules are applied. Because filesystem rules do not constrain shell execution, the control plane also refuses to publish `.deepagents/**` and deep-agent config paths after either Git landing path.
- `harness_profile`, `filesystem_permissions`, and `subagents` inputs are strict-validated and workflow-owned. Specialist subagents may only add filesystem deny rules, use repository-local skills, and select an explicit allow-list of MCP tools already loaded for the action.
- The filesystem permission rules do not sandbox shell execution. Keep using the command allow-list, deny-list, protected paths, and secret-free environment; a real container/jail is required for hard process isolation.
...
## 7. Scoped tokens & least privilege
...
- [ ] Keep `require_push_approval: true` unless direct branch updates are explicitly intended.
- [ ] Add `protected_paths` for CI workflows, release configuration, or other repository-specific control-plane files.
