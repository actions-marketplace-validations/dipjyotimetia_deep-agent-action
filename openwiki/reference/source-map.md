...
| `stream.ts`      | Streaming agent driver (`runAgentStream`). Progress mirroring, activity tracking, budget/timeout abort, and stalled-loop recovery.                                         |
...
| `policy.ts`      | Parses/validates harness profiles and filesystem permissions. Discovers `.deepagents/` sources and builds security-floor permissions.                                      |
...
## `packages/create-deep-agent-action/` — Public setup creator

| File                               | Purpose                                                                              |
| ---------------------------------- | ------------------------------------------------------------------------------------ |
| `bin/create-deep-agent-action.mjs` | Node-compatible `npx` installer for a pinned starter workflow and optional guidance. |
| `package.json`                     | Independently publishable package metadata and CLI entrypoint.                       |
...
| `publish-creator.yml`       | Publishes the public setup creator with npm provenance. |
